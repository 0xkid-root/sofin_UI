        ...
        (counters.forEach((n, e) => {
          ...
          const a = () => {
            var e = +n.getAttribute("data-value"),
              t = +n.innerHTML;
            t < e
              ? ((n.innerHTML = `${Math.ceil(t + e / 5e3)}`), setTimeout(a, 1))
              : (n.innerHTML = e);
          };
          a(),
...
let company_bar = document.querySelector("#company_bar"),
  companyBarItem = document.querySelectorAll(".companyBar");
if (company_bar) {
  let e = new IntersectionObserver(
    (e, t) => {
      var [e] = e;
      e.isIntersecting &&
        (companyBarItem.forEach((n, e) => {
          n.innerHTML = "0";
          const a = () => {
            var e = +n.getAttribute("data-value"),
              t = +n.innerHTML;
          ...
          a(),
            n.parentElement.style.animation
              ? (n.parentElement.style.animation = "")
              : (n.parentElement.style.animation = `slide-up 0.3s ease forwards ${
                  e / companyBarItem.length + 1.5
                }s`);
        }),
        t.unobserve(company_bar));
    },
    { root: null, threshold: 1 }
  );
  e.observe(company_bar);
}
